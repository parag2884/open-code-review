...
필터는
[`internal/agent/selection.go`](https://github.com/parag2884/open-code-review/blob/main/internal/agent/selection.go)에
있는 여섯 관문 알고리즘입니다. diff마다 OCR이 다음을 묻습니다.

1. **`binary`** — 바이너리 파일인가? 그렇다면 제외.
2. **`secret_exclude`** — 이전 경로나 새 경로가 내장 시크릿 경로 보호 대상인가? 조건 없이 적용되는 glob 패턴은 [`default_secret_patterns.json`](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/default_secret_patterns.json)에 있습니다. 그렇다면 제외.
...
5. **`unsupported_ext`** — 파일 확장자가
   [허용 목록](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/supported_file_types.json)에
   있는가? 없으면 제외.
...
앞에서, 파일별 필터가 돌기 전
[`internal/diff/git.go`](https://github.com/parag2884/open-code-review/blob/main/internal/diff/git.go)의
diff 단계에서 일어납니다.
