...
1. **`binary`**——文件是二进制吗？排除。
2. **`secret_exclude`**——旧路径或新路径是否命中内置敏感路径保护？无条件匹配的 glob 模式列在 [`default_secret_patterns.json`](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/default_secret_patterns.json) 中。若是，排除。
...
5. **`unsupported_ext`**——文件扩展名在
   [白名单](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/supported_file_types.json)
   里吗？不在则排除。
