...
2. **`secret_exclude`** — Does either path match a
   built-in secret-path protection? The unconditional glob patterns are listed in [`default_secret_patterns.json`](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/default_secret_patterns.json).
...
5. **`unsupported_ext`** — Is the file extension in the
   [allowlist](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/supported_file_types.json)?
   Excluded if not.
