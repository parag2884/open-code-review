...
フィルタリングは 6 段階のゲートアルゴリズムで、[`internal/agent/selection.go`](https://github.com/parag2884/open-code-review/blob/main/internal/agent/selection.go) にあります。各 diff について、OCR は順に次を問います:

1. **`binary`**: ファイルはバイナリか？ 除外します。
2. **`secret_exclude`**: 古いパスまたは新しいパスが組み込みのシークレットパス保護の対象か？ 無条件に適用される glob パターンは [`default_secret_patterns.json`](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/default_secret_patterns.json) にあります。対象なら除外します。この保護はユーザールールより先に適用され、`include` パターンでは上書きできません。
...
5. **`unsupported_ext`**: ファイルの拡張子は[ホワイトリスト](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/supported_file_types.json)にあるか？ なければ除外します。
...
組み込みの除外リスト（[`internal/config/allowlist/default_exclude_patterns.json`](https://github.com/parag2884/open-code-review/blob/main/internal/config/allowlist/default_exclude_patterns.json) を参照）は、テストファイルのパターンに一致します:
...
ノイズディレクトリのフィルタリング（`vendor/`、`node_modules/`、`target/`……）は、より早い段階、[`internal/diff/git.go`](https://github.com/parag2884/open-code-review/blob/main/internal/diff/git.go) の diff 層で発生し、ファイルごとのフィルタリングより先に実行されます。
